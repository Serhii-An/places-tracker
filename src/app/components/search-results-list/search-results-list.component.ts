...
export class SearchResultsListComponent {
  ...
  readonly selectedSortType = signal<'distance' | 'default' | 'name'>('default');
  ...
  readonly filteredPlaces = computed(() => {
    ...
    const itemsToSort = [...places];

    switch (this.selectedSortType()) {
      case('distance'):
        return itemsToSort.sort((a, b) => (a.distance ?? 0) - (b.distance ?? 0));
      case('name'):
